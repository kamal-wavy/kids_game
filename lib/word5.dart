...
class CrosswordWidget5 extends StatefulWidget {
  CrosswordWidget5({Key? key}) : super(key: key);

  @override
  _CrosswordWidgetState createState() => _CrosswordWidgetState();
}

class _CrosswordWidgetState extends State<CrosswordWidget5> {
  int numBoxPerRow = 6;
  double padding = 5;
  ...
  List<String> generateRandomWords() {
    ...
    allWords.shuffle();
    return allWords.where((word) => word.length <= 7).take(6).toList();
    // return allWords.take(6).toList();
  }
  ...
  void generateRandomWord() {
    ...
    final int longestWordLength =
        wl.map((word) => word.length).reduce((a, b) => a > b ? a : b);

    // Set the grid size to the length of the longest word plus some padding
    numBoxPerRow = (longestWordLength + 2).clamp(6, 10);
